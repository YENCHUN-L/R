---
title: "2382 Quanta Monthly Sales"
output:
...
### Line plot
From the plot we can see that there¡¦s fluctuation from 2015 to 2017.There are four major fluctuation. Through the plot we are not able to see if there is seasonal effect or not.
We will build seasonal plot and month plot after.
```{r plot whole data, echo=TRUE}
plot(to, main="Sales")
```

### Season and month plot
From the seasonal and month plot we can see that Feb. is generally the lowest and from Feb. will have a upward fluctuation to Dec.
...
With the plot under we can see that the fluctuation is clearly not enough to the fact.
...
The residual diagnostics show that after and below lag 9 the residuals of this method are not white noise.
...
The Holt-Winters multiplicative method have the best performance compare to others.
Where it does not have an acceptable residual diagnostics.
Therefore we have to go with Damped Holt-winters multiplicative method, which it have the second best of accuracy with residual diagnostics that is acceptable.
...
Due to the fact that we do see a continues pattern of fluctuation.
...
The residual diagnostics doesn't have an acceptable result.
So we have to go with ETS MMM with damped.
...
The auto.arima procedure results in an ARIMA(1,0,0)(1,1,0) model.
This model does not shows satisfactory diagnostics.
...
The first difference applyed suggest to take zero difference, then the seasonal difference suggest zero
differences.
...
With the table we can see that Arima auto test have best performance overall of MAE, MAPE and MASE.
The residual diagnostics results of Arima auto test is also acceptable.
Therefore we will use Arima auto test model as final to do forecast to 2020.
